...
interface EditZonePermissionFormProps {
  permission: any;
  platformUsers: Array<{ id: number; name: string; email: string }>;
  zones: Array<{ id: number; name: string }>;
}

export function EditZonePermissionForm({ permission, platformUsers, zones }: EditZonePermissionFormProps) {
  const router = useRouter();
  const params = useParams();
  const locale = (params?.locale as string) || 'en';
  const [isPending, startTransition] = useTransition();
  const [selectedZoneId, setSelectedZoneId] = useState<number | null>(permission.resource_id ?? null);
  ...
  return (
    <Card className="p-4 sm:p-6">
      <h2 className="text-lg font-semibold mb-4">Edit Zone Type Permission</h2>
