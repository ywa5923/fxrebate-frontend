...
interface EditBrokerPermissionFormProps {
  permission: any;
  platformUsers: Array<{ id: number; name: string; email: string }>;
  brokers: Array<{ id: number; trading_name: string }>;
}

export function EditBrokerPermissionForm({ permission, platformUsers, brokers }: EditBrokerPermissionFormProps) {
  const router = useRouter();
  const params = useParams();
  const locale = (params?.locale as string) || 'en';
  const [isPending, startTransition] = useTransition();
  const [selectedBrokerId, setSelectedBrokerId] = useState<number | null>(permission.resource_id ?? null);
  ...
  return (
    <Card className="p-4 sm:p-6">
      <h2 className="text-lg font-semibold mb-4">Edit Broker Permission</h2>
