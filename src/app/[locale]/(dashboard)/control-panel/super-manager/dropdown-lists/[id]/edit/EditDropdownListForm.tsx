...
import { updateDropdownList } from '@/lib/dropdown-list-requests';
import type { DropdownList, DropdownListOption } from '@/types/DropdownList';
...
interface EditDropdownListFormProps {
  initial: DropdownList;
}

export default function EditDropdownListForm({ initial }: EditDropdownListFormProps) {
  const router = useRouter();
  const params = useParams();
  const locale = (params?.locale as string) || 'en';
  const [isPending, startTransition] = useTransition();

  const [listName, setListName] = useState(initial.name || '');
  const [description, setDescription] = useState(initial.description || '');
  const [options, setOptions] = useState<OptionRow[]>(
    (initial.options || []).map((o: DropdownListOption) => ({ label: o.label, value: o.value, valueAuto: false }))
  );
  ...
  const updateOption = (index: number, field: 'label' | 'value', value: string) => {
    setOptions((prev) => prev.map((opt, i) => {
      ...
      if (field === 'label') {
        ...
        return { ...opt, label: nextLabel, value: nextValue };
      }
      if (field === 'value') {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const res = await updateDropdownList(initial.id, listName.trim(), description.trim() || null, cleanedOptions);
      if (res.success) {
        toast.success('List updated');
        router.push(`/${locale}/control-panel/super-manager/dropdown-lists`);
        router.refresh();
      } else {
        toast.error('Failed to update list', { description: res.message });
      }
    });
  };
