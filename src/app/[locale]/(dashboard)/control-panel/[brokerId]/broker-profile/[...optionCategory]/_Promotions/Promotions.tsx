...
export default function Promotions({
  ...
  const [activeTab, setActiveTab] = useState<string>(
    promotions?.[0]?.id?.toString() || "",
  );
  const [showNewPromotion, setShowNewPromotion] = useState(false);
  const [confirmDeletePromotion, setConfirmDeletePromotion] = useState<
    number | null
  >(null);
  ...
  async function handleDeletePromotion(promotionId: number) {
    try {
      ...
      if (response.success) {
        ...
        thisLogger.error("Failed to delete promotion", {
          error: response.message,
          context: { promotionId, broker_id },
        });
      }
    } catch (error) {
    ...
    <div className="container mx-auto px-2 sm:px-6 pt-6 pb-6">
      ...
      {promotions && promotions.length > 0 ? (
        <>
              ...
              {promotion.option_values && promotion.option_values.length > 0 ? (
                  ...
                  <div className="flex items-center justify-end gap-2 mt-2 mb-1">
                    {can_manage && (
                      <Button
                        ...
                        onClick={() =>
                          setConfirmDeletePromotion(promotion.id)
                        }
                ...
                <div className="text-center py-12 bg-gray-50 dark:bg-gray-900/50 rounded-lg border-2 border-dashed border-gray-200 dark:border-gray-700">
                  ...
                  <p className="text-sm text-gray-400 dark:text-gray-500 mt-1">
                    This promotion has no option values to configure.
                  </p>
                </div>
              )}
            </div>
          ...
          <Dialog
            open={!!confirmDeletePromotion}
            onOpenChange={(open) => {
              if (!open) setConfirmDeletePromotion(null);
            }}
          >
            <DialogContent>
              <DialogHeader>
              ...
              <DialogFooter>
                <Button
                  variant="outline"
                  onClick={() => setConfirmDeletePromotion(null)}
                >
                  Cancel
                </Button>
                <Button
        ...
        !showNewPromotion && (
          <NotFoundEntity
            ...
            description={
              can_manage
                ? "Click here or use the + button to add a promotion."
                : "No promotions are configured for this broker yet."
            }
            onClick={
              can_manage ? () => setShowNewPromotion(true) : undefined
            }
