...
export function CustomSelectBox({
  ...
  const prevCheckedItemsRef = useRef<string[]>(checkedItems);
  // useEffect(() => {
  ...
  useEffect(() => {
    if (JSON.stringify(prevCheckedItemsRef.current) !== JSON.stringify(checkedItems)) {
      prevCheckedItemsRef.current = checkedItems; // Update reference
      updateURL(); // Run only when checkedItems truly changes
    }
