...
export default async function Home({children,zone,locale}:{children?:React.ReactNode,zone:string,locale:string}) {


  const { title: paymentTitle, methods } = ourPaymentMethods;
  const { title: partnersTitle, items: partnersItems } = ourPartners;

  const _t=await getTranslations(locale,zone,"home_page","client");
